...
('job_cost_report', 'total_overunder_identity',
 'Over/Under = JTD - Budget',
 'Total over/under must equal total JTD cost minus total revised budget',
 1, 'extraction_error', 'doc',
...
('job_cost_report', 'overunder_identity',
 ...
 '(() => { const bud = ctx.current.revised_budget || 0; const jtd = ctx.current.jtd_cost || 0; const ou = ctx.current.over_under_budget; if (ou == null) return { pass: true, message: "No over/under value" }; const expected = bud - jtd; const delta = Math.abs(ou - expected); return { pass: delta <= 0.01, expected, actual: ou, delta, message: delta <= 0.01 ? "OK" : "code " + ctx.current.cost_code + ": ou=" + ou.toFixed(2) + " vs (bud-jtd)=" + expected.toFixed(2) }; })()',
 0.01, '{over_under_budget}',
 NULL),

('job_cost_report', 'code999_all_signs_negative',
 'Code 999 Values Negative',
 'Revenue code 999 should have negative values for all budget and cost columns',
 1, 'extraction_error', 'doc',
 ...
 0, '{original_budget,revised_budget,jtd_cost}',
 'Code 999 (revenue) should have all negative values in Sage JCRs.'),
