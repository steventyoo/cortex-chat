-- Fix: handle LLM-quoted placeholders like '{{project_id}}' and '{{org_id}}'
-- The LLM sometimes wraps the placeholder in its own single quotes, and then
-- quote_literal adds another layer, producing ''value'' which is a syntax error.
-- Solution: replace the quoted variant first (stripping LLM quotes), then the bare variant.
...
BEGIN
  ...
  -- Replace quoted variants first: '{{org_id}}' → 'value' (quote_literal already adds quotes)
