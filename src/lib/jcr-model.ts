...
function buildOverview(
  ...
  const revenue = Math.abs(num(fields['total_revenues'] || fields['sales_cost_code_rev_budget']));
  ...
  rows.push(row(t, s, k, 'source_pr', 'source_pr', 'Source: Payroll', 'currency', 'Extracted', num(fields['pr_amount']), null));
  rows.push(row(t, s, k, 'source_ap', 'source_ap', 'Source: AP', 'currency', 'Extracted', num(fields['ap_amount']), null));
  rows.push(row(t, s, k, 'source_gl', 'source_gl', 'Source: GL', 'currency', 'Extracted', num(fields['gl_amount']), null));
...
function buildCostBreakdown(records: RecordRow[], fields: FieldsMap, meta: ProjectMeta): ExportRow[] {
  ...
  const pr = num(fields['pr_amount']);
  const ap = num(fields['ap_amount']);
  const gl = num(fields['gl_amount']);
...
function buildBenchmarkKPIs(records: RecordRow[], fields: FieldsMap, meta: ProjectMeta): ExportRow[] {
  ...
  if (revenue > 0) {
    ...
    const glCost = num(fields['gl_amount']);
    if (glCost > 0) {
  ...
  // Vendor KPIs
  const apTotal = num(fields['ap_amount']);
